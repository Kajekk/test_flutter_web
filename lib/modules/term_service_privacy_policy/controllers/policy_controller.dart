...
class PolicyController extends SubTabController {
  PolicyController({required repository, required this.info}) : _repository = repository;

  final IAgreementAndPolicyRepository _repository;
  final SubTabInfoModel info;
  final _stateStream = PolicyState().obs;

  get state => _stateStream.value;

  void fetchListItems(QueryModel queryModel) async {
    _stateStream.value = PolicyLoading();
    var res = await _repository.getPolicies(queryModel);
    if (res.status != ApiStatus.Ok) {
      _stateStream.value = PolicyFailure(
          message: res.message ?? "Something went wrong, please try again");
      return;
    }
    totalRows = res.total ?? 0;
    _stateStream.value = PolicyLoaded(listData: res.data!);
    dataList = res.data!;
  }
  ...
  void changeSubTab() {
    isCurrent = true;
    Get.find<AgreementController>().isCurrent = false;
  }
